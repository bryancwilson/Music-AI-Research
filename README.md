...
#### Embedding
I constructed a multilayer perceptron to predict the dissonance values of all possible binary chord vector representations. I included a 50-dimensional embedding layer in the architecture to extract after the model had been trained. After the model overfit to the data (this is desirable because we don't want it to generalize to new data but rather memorize the training set), I extracted the embedding layer and performed certain dimensionality reduction techniques to obtain the graph below. The x and y axis for this graph are purely positional and relative. Note that there are noticeable clusters of certain chords due to their similar dissonance values. Now, using the k-nearest neighbor algorithm, I am able to enter any continous vector and extract the nearest chord in the embedding space. For example, I can find find a chord based on the average between the dissonance value of Cmajor and Eminor.
